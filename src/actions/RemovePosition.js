...
async function handleWalletsWithPosition(walletsWithPosition, poolAddress) {
    ...
    console.log("\n\x1b[31m~~~ [!] | ERROR | Following wallets still have positions:\x1b[0m");
    walletsWithPosition.forEach(wallet => console.log(`- ${wallet.description}`));
    
    const action = await question("\nSelect action:\n1. Recheck positions\n2. Remove liquidity again\n3. Skip\n\n[...] Your choice (1-3): ");
    ...
    if (action === "1" || action === "2") {
        ...
        const checkPromises = walletsWithPosition.map(async wallet => {
            if (action === "2") {
                try {
                    ...
                    console.error(`\x1b[31m~~~ [!] | ERROR | [${wallet.description.slice(0, 4)}...] | Error retrying liquidity removal: ${error.message}\x1b[0m`);
                }
            }
        ...
        if (remainingWallets.length > 0) {
            ...
            console.log(`\n\x1b[36m[${new Date().toLocaleTimeString()}] | SUCCESS | All positions successfully removed\x1b[0m`);
            return [];
        }
    }
    
    return walletsWithPosition; // Return original list if skip is selected
}
...
export async function handleRemovePosition(selectedWallets, predefinedPool = null) {
    try {  
        ...
        if (poolCheck) {   
            ...
            console.log(`\n\x1b[36m[${new Date().toLocaleTimeString()}] | SUCCESS | Position removal completed\x1b[0m`);
        ...
        if (error.message) {
            ...
            console.error(`\x1b[31m~~~ [!] | ERROR | Unknown error while removing position\x1b[0m`);
        }
        returnToMainMenu();
    }
} 