...
async function handleWalletsWithoutPosition(walletsWithoutPosition, poolAddress, solAmount, strategy = '2') {
    ...
    if (action === "1") {
        console.log("\n\x1b[36m[⌛] | WAITING | Waiting 2 seconds before rechecking...\x1b[0m");
        await delay(2000);
        ...
        if (remainingWallets.length > 0) {
            ...
            console.log("\n\x1b[36m[${new Date().toLocaleTimeString()}] | SUCCESS | All positions have been rechecked\x1b[0m");
            return [];
        }
    } else if (action === "2") {
        const retryPromises = walletsWithoutPosition.map(async wallet => {
...
export async function handleOpenPosition(selectedWallets, predefinedPool = null, predefinedAmount = null) {
    try {
        ...
        console.log(`\n\x1b[36m[${new Date().toLocaleTimeString()}] | SUCCESS | Position opening completed\x1b[0m`);
        
        // Use updated number of problematic wallets
