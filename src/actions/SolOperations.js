...
export async function handleSolDistribution(MainWallet, selectedWallets) {
    try {
        if (selectedWallets.length < 1) {
            console.error(`\x1b[31m~~~ [!] | ERROR | Must select at least 1 wallet\x1b[0m\n`);
            returnToMainMenu();
        }
        ...
        console.error(`\x1b[31m~~~ [!] | ERROR | Error distributing SOL: ${error.message}\x1b[0m`);
        returnToMainMenu();
    }
}
...
export async function handleSolConsolidation(MainWallet, selectedWallets) {
    try {
        if (selectedWallets.length < 2) {
            console.error(`\x1b[31m~~~ [!] | ERROR | Must select at least 2 wallets\x1b[0m\n`);
            returnToMainMenu();
        }
        ...
        const consolidationPromises = selectedWallets
            ...
            .map(sourceWallet => 
                consolidateSol(sourceWallet, targetWallet)
                    .catch(error => {
                        console.error(`\x1b[31m~~~ [!] | ERROR | [${sourceWallet.description.slice(0, 4)}..] Error consolidating SOL: ${error.message}\x1b[0m`);
                        returnToMainMenu();
                    })
            );
        ...
        console.error(`\x1b[31m~~~ [!] | ERROR | Error consolidating SOL: ${error.message}\x1b[0m`);
        returnToMainMenu();
    }
}
