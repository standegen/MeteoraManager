...
export async function handleTokenConsolidation(MainWallet, selectedWallets) {
    try {
        ...
        while (!consolidationComplete) {
            console.log(`\n\x1b[36m[⌛] | WAITING | [${targetWallet.description.slice(0, 4)}..] Token consolidation\x1b[0m`);
            ...
            const consolidationPromises = selectedWallets
                ...
                .map(sourceWallet => 
                    consolidateTokens(sourceWallet, targetWallet)
                        .catch(error => {
                            console.error(`\x1b[31m~~~ [!] | ERROR | [${sourceWallet.description.slice(0, 4)}..] Consolidation error: ${error.message}\x1b[0m`);
                            returnToMainMenu();
                        })
                );
            ...
            if (hasRemainingTokens) {
                const retryChoice = await question("\n[...] Remaining tokens found. Select action (1: Retry consolidation, 2: Continue, 3: Finish): ");
                if (retryChoice === "1") {
        ...
        console.error(`\x1b[31m~~~ [!] | ERROR | Error processing tokens: ${error.message}\x1b[0m`);
        returnToMainMenu();
    }
} 