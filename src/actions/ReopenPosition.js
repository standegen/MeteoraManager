...
export async function handleReopenPosition(selectedWallets) {
    try {        
        console.log("\nHOW DO YOU WANT TO REOPEN THE POSITION:\n=========================");
        ...
        console.log("\n\x1b[36m[⌛] | WAITING | Checking position closure...\x1b[0m\n");
        await delay(2000);
        ...
        // Handle wallets without positions
        let finalWalletsWithoutPosition = [];
        if (walletsWithoutPosition.length > 0) {
            console.log("\n\x1b[31m~~~ [!] | ERROR | Following wallets require attention:\x1b[0m");
            walletsWithoutPosition.forEach(wallet => 
                console.log(`- ${wallet.description.slice(0, 4)}...`)
            );

            console.log("\n\x1b[36m[!] | ADVICE | Try checking positions 1-2 times, if positions don't appear, then add liquidity again\x1b[0m\n");
                ...
                const retryPromises = walletsWithoutPosition.map(async wallet => {
                    try {
                        ...
                        if (!position) {
                            ...
                            console.log(`\n\x1b[36m[${new Date().toLocaleTimeString()}] | SUCCESS | [${wallet.description.slice(0, 4)}...] | Position successfully created\x1b[0m`);
                            return null;
                        }
                    } catch (error) {
        ...
        console.log("\n\x1b[36m• Position reopening completed\x1b[0m");
